...
class CustomConv2DFunction(Function):
    ...
    def forward(ctx, input_feats, weight, bias, stride=1, padding=0):
        ...
        output_h = (input_feats.size(2) + 2 * ctx.padding - kernel_size) // ctx.stride + 1
        output_w = (input_feats.size(3) + 2 * ctx.padding - kernel_size) // ctx.stride + 1

        unfold = nn.Unfold(kernel_size=kernel_size, padding=ctx.padding, stride=ctx.stride)

        #2images,3channels,12x12
        #7x7 kernel
        unfold_input = unfold(input_feats)
        ...
        #64x147
        unfold_output = unfold_weight.matmul(unfold_input)
        #2x64x36


        temp = unfold_output.transpose(0, 1)
        #64x2x36
        ...
        for i in range(temp.size(0)):
            temp[i] += bias[i]
        unfold_output = temp.transpose(0, 1)
        #2x64x36


        unfold1 = nn.Fold(output_size=(output_h, output_w), kernel_size=(1,1))

        output = unfold1(unfold_output)
        #2x64x6x6
    ...
    def backward(ctx, grad_output):
        ...
        #################################################################################
        # Fill in the code here
        unfold = nn.Unfold(kernel_size=(1,1))
        
        unfold_weight = weight.view(weight.size(0), -1)
        #64x147

        unfold_weight = unfold_weight.transpose(0, 1)
        #147x64
        unfold_grad = unfold(grad_output)
        #grad_output 2x64x6x6
        #unfold_grad 2x64x36

        unfold_grad_input = unfold_weight.matmul(unfold_grad)
        #2x147x36
        fold_grad_input = nn.Fold(output_size=(input_height, input_width), kernel_size=kernel_size, stride=stride, padding=padding)
        grad_input = fold_grad_input(unfold_grad_input)
        #2x3x12x12

        unfold_grad_weight = unfold_grad.matmul(unfold_input.transpose(1,2))
        grad_weight = unfold_grad_weight.sum(dim=0)
        grad_weight = grad_weight.view(weight.size())
        #64x3x7x7
        #################################################################################
